...
class WC_Gateway_CityPay extends WC_Payment_Gateway
{
    ...
    {
        $this->context = get_file_data(__DIR__ . '/wc-payment-gateway-citypay.php', []);
        $this->id = 'citypay';
...
//        $this->supports = array('products');
        ...
        $this->postback_base = $this->get_option('postback_base');
        ...
        $this->postback_url = trailingslashit($this->postback_base) . 'wc-api/citypay-postback';
    ...
    function check_postback()
    {
        ...
        if (isset($_GET['pl_orderkey'])) {
            ...
            try {
                ...
                if (!$this->validatePostbackDigest($postback_data)) {
                    $this->warningLog('Unable to validate postback data');
                    throw new Exception('Unable to validate postback data');
                }

                $this->infoLog('Postback data is valid');
...
//                    $this->debugLog('isAuthorised result is type ' . gettype($result) . ' value = ' . $result);
                    ...
                    if ($result) {
                        ...
                        if (!empty($postback_data['maskedpan'])) {
                            $mp = $postback_data['maskedpan'];
                            if (!empty($postback_data['cardscheme'])) {
                                $mp = $postback_data['cardscheme'] . '/' . $mp;
                            }
                            $mp = $mp . ' ' . $postback_data['expyear'] . '/' . str_pad($postback_data['expmonth'], 2, '0', STR_PAD_LEFT);
                            update_post_meta($order->get_id(), 'Card used', $mp);
                        }
                ...
                wp_die("CityPay Postback Error: " . $e->getMessage());
            }
        }
    }
